...
def test_qualean_repr():
    user_input = random.choice([-1, 0, 1])
    q = session4.Qualean(user_input)
    assert (
        q.__repr__() == f"Qualean: {user_input}"
    ), "The representation of the Qualean object does not meet expectations"


def test_qualean_str():
    user_input = random.choice([-1, 0, 1])
    q = session4.Qualean(user_input)
    assert q.__str__() == f"Qualean String for number: " + str(
        q.user_choice
    ), "The print of the Qualean object does not meet expectations"
...
def test_qualean_decimal_precision():
    user_input = random.choice([-1, 0, 1])
    q = session4.Qualean(user_input).return_qualean()
    if q != 0:
        assert (
            len(str(q).split(".")[-1]) == 10
        ), "Rounding to specified precision is not happening"
    else:
        assert (
            len(str(q).split(".")[-1]) == 1
        ), "Rounding to specified precision is not happening"
...
def test_100_qualeans():
    ...
    for i in range(100):
        sum = sum + q
    assert round(sum, 10) == round(
        100 * q, 10
    ), "Test case for 100 times Qualean failed"
...
def test_function_sqrt():
    ...
    if q.return_qualean() >= 0:
        assert round(q.__sqrt__(), 10) == round(
            Decimal(q.return_qualean()).sqrt(), 10
        ), "Test case for sqrt method failed..."
    else:
        assert (
            q.__sqrt__() == str(round(Decimal(q.__invertsign__()).sqrt(), 10)) + "i"
        ), "Qualean sqrt failed for negative qualeans..."
...
def test_million_qualeans_sum():
    ...
    for i in range(1000000):
        user_input = random.choice([-1, 0, 1])
        sum = sum + session4.Qualean(user_input).return_qualean()
    assert (
        math.isclose(sum, 0.0) == False
    ), "Test case for MILLION Qualeans addition failed"
...
def test_million_qualeans_mul():
    ...
    for i in range(1000000):
        ...
        sum = sum * session4.Qualean(user_input).return_qualean()
    assert (
        math.isclose(sum, 0.0) == True
    ), "Test case for MILLION Qualeans multiplication failed"
...
def test_qualean_validity():
    user_input = random.choice([-1, 0, 1])
    q1 = session4.Qualean(user_input)
    q2 = session4.Qualean(1)
    q3 = session4.Qualean(-1.0)
    assert (
        isinstance(q1, session4.Qualean) and q1.__float__() < 1 and q1.__float__() > -1
    ), "Not generated valid Qualean..."
    assert (
        isinstance(q2, session4.Qualean) and q2.__float__() < 1 and q1.__float__() > -1
    ), "Not generated valid Qualean..."
    assert (
        isinstance(q3, session4.Qualean) and q3.__float__() < 1 and q1.__float__() > -1
    ), "Not generated valid Qualean..."
...
def test_function_and():
    user_input = random.choice([-1, 0, 1])
    q1 = session4.Qualean(user_input)
    q2 = session4.Qualean(user_input)
    if q1.return_qualean() == 0 or q2.return_qualean() == 0:
        assert (
            q1.__and__(q2) == False
        ), "Qualean and operation failed when one of them is false..."
    else:
        assert (
            q1.__and__(q2) == True
        ), "Qualean and operation failed when none of them is false.."
...
def test_function_or():
    ...
    if q1.return_qualean() == 0 and q2.return_qualean() == 0:
        assert (
            q1.__or__(q2) == False
        ), "Qualean or operation failed when both qualeans are false..."
    else:
        assert (
            q1.__or__(q2) == True
        ), "Qualean or operation failed when at least one qualean is true..."
...
def test_function_add():
    ...
    q2 = session4.Qualean(user_input)
    assert (
        q1.__add__(q2) == q1.return_qualean() + q2.return_qualean()
    ), "This is how you mess up simple addition..."
...
def test_function_add_non_qualean():
    ...
    assert (
        q1.__add__(q2) == q1.return_qualean() + q2
    ), "This is how you mess up simple addition..."
...
def test_function_mul():
    ...
    q2 = session4.Qualean(user_input)
    assert (
        q1.__mul__(q2) == q1.return_qualean() * q2.return_qualean()
    ), "This is how you mess up simple multiplication..."
...
def test_function_mul_non_qualean():
    ...
    q2 = random.uniform(-1000, 1000)
    assert (
        q1.__mul__(q2) == q1.return_qualean() * q2
    ), "This is how you mess up simple multiplication..."
...
def test_function_ge():
    ...
    q2 = session4.Qualean(user_input)
    assert q1.__ge__(q2) == (
        q1.return_qualean() >= q2.return_qualean()
    ), "Qualean ge method failed. q1 should be greater than equal to q2"
...
def test_function_ge_non_qualean():
    ...
    q2 = random.uniform(-1000, 1000)
    assert q1.__ge__(q2) == (
        q1.return_qualean() >= q2
    ), "Non Qualean ge method failed. q1 should be greater than equal to q2"
...
def test_function_gt():
    ...
    assert q1.__gt__(q2) == (
        q1.return_qualean() > q2.return_qualean()
    ), "Qualean gt method failed. q1 should be greater than q2"
...
def test_function_gt_non_qualean():
    ...
    assert q1.__gt__(q2) == (
        q1.return_qualean() > q2
    ), "Non Qualean gt method failed. q1 should be greater than q2"
...
def test_function_le():
    ...
    assert q1.__le__(q2) == (
        q1.return_qualean() <= q2.return_qualean()
    ), "Qualean le method failed. q1 should be less than equal to q2"
...
def test_function_le_non_qualean():
    ...
    assert q1.__le__(q2) == (
        q1.return_qualean() <= q2
    ), "Non Qualean le method failed. q1 should be less than equal to q2"
...
def test_function_lt():
    ...
    assert q1.__lt__(q2) == (
        q1.return_qualean() < q2.return_qualean()
    ), "Qualean lt method failed. q1 should be less than q2"
...
def test_function_lt_non_qualean():
    ...
    assert q1.__lt__(q2) == (
        q1.return_qualean() < q2
    ), "Non Qualean lt method failed. q1 should be less than q2"
...
def test_or_function_both_undefined():
    r2 = session4.Qualean(0)
    r1 = session4.Qualean(0)
